...
public class EntityAIWorkFarmer extends AbstractEntityAIInteract<JobFarmer>
{
    ...
    private AIState prepareForFarming()
    {
        ...
        if (currentField.needsWork())
        {
            ...
            else if (currentField.getFieldStage() == Field.FieldStage.HOED && !needsToolOrWeapon(ToolType.HOE))
            {
    ...
    private AIState workAtField()
    {
        ...
        if (buildingFarmer == null || needsToolOrWeapon(ToolType.HOE) || buildingFarmer.getCurrentField() == null)
        {
            return PREPARING;
        }
    ...
    {
        if (shouldHoe(position) && !needsToolOrWeapon(ToolType.HOE))
        {
