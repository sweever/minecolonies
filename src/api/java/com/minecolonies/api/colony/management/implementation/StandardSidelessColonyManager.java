...
{
    Map<World, IWorldColonyController<B, C>> worldColonyControllerMap  = new HashMap<>();
    ...
    {
        return ImmutableList.copyOf(worldColonyControllerMap.values());
    }

    @Override
    public void onWorldLoad(@NotNull final WorldEvent.Load event)
    {
        //Lets load the controller just to be sure.
        getControllerForWorld(event.getWorld());
    ...
    public void onWorldUnload(@NotNull final WorldEvent.Unload event)
    {
        ...
        //Since the world is unloaded, lets unregister it.
        worldColonyControllerMap.remove(event.getWorld());
    }
}
